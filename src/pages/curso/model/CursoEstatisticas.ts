...
export default class CursoEstatisticas {
  ...
  qtdeDeAulasConcluidas() {
    return this.capitulos.reduce(
      (qtde, cap) => qtde + cap.aulas.filter((aula) => aula.concluida).length,
      0
    );
  }
  ...
  duracaoTotalEmSegundos() {
    return this.capitulos.reduce(
      (duracao, cap) =>
        duracao +
        cap.aulas.reduce((duracao, aula) => duracao + aula.duracao, 0),
      0
    );
  }
  ...
  duracaoTotal() {
    ...
    duracaoConcluidaEmSegundos() {
      return this.capitulos.reduce(
        (duracao, cap) =>
          duracao +
          cap.aulas.reduce((duracao, aula) => duracao + (aula.concluida? aula.duracao : 0), 0),
        0
      );
    }
